...
// Forward declare or include any other headers your existing geometry.h requires
...
// GPU buffer descriptor expected by Dejun.cu
struct gpu_info {
    float            *d_data;
    uint32_t         *d_os;
    struct result_container *d_intersect;
};

// Result structure matching Dejun.cu
struct result_container {
    bool     intersected;
    float    distance;
    uint32_t p1;
    uint32_t p2;
    // ... any additional fields present in Dejun.cu ...
};

// C-linkage declaration for the batch intersection API
// This will now match the extern "C" wrapper in Dejun.cu
void TriInt_batch_gpu(
    gpu_info         *gpu,
    const float      *data,
    const uint32_t   *offset_size,
    const float      *hausdorff,
    result_container *intersection,
    uint32_t          pair_num,
